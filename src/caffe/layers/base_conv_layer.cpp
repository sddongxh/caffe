...
void BaseConvolutionLayer<Ftype, Btype>::LayerSetUp(const vector<Blob*>& bottom,
  ...
  for (int i = 0; i < num_spatial_axes_; ++i) {
    dilation_data[i] = (num_dilation_dims == 0) ? kDefaultDilation : conv_param.dilation(
...
template<typename Ftype, typename Btype>
void
BaseConvolutionLayer<Ftype, Btype>::Reshape(const vector<Blob*>& bottom, const vector<Blob*>& top) {
  const int first_spatial_axis = channel_axis_ + 1;
...
INSTANTIATE_CLASS_FB(BaseConvolutionLayer);
